...
def main():
    ...
    for path in HUMAN_EVAL_DIR_PATH.glob("*.json"):
        ...
        if path.name in TEST_SET_FILE:
            ...
            single_df["dialogue_id"] = single_df["dialogue_id"].apply(lambda x: x.replace("dstc10-task5.1", path.stem))
    ...
    if df["dialogue_id"].duplicated().any():
        raise ValueError(f"There are duplicate dialogue_ids in the human evaluation data.\n"
    ...
    df["annotations_average"] = df["annotations"].apply(lambda x: {k: np.mean(v) for k, v in x.items()})
    ...
    for prediction_path in Path(HUMAN_EVAL_DIR_PATH).glob("submission_*.csv"):
        ...
        prediction_df["eval_aspect"] = prediction_df["dialogue_id"].apply(lambda x: x.rsplit("|", 1)[1])
        prediction_df["dialogue_id"] = prediction_df["dialogue_id"].apply(lambda x: x.rsplit("|", 1)[0])

        # test set id mapping
        prediction_df["dialogue_id"] = prediction_df["dialogue_id"].apply(lambda x: testset_id_mapping.get(x, x))
        ...
        prediction_df = prediction_df.pivot(index="dialogue_id", columns="eval_aspect", values="score").add_prefix("prediction_")
        ...
        if not set(prediction_df.index).issubset(set(df.index)):
            raise ValueError(f"The dialogue_ids in the prediction file {prediction_path} are not a subset of the dialogue_ids in the human evaluation data.\n"
        ...
        prediction_df["_id"] = prediction_df["dialogue_id"].apply(lambda x: x.rsplit("_", 1)[1])
        prediction_df["dataset"] = prediction_df["dialogue_id"].apply(lambda x: x.rsplit("_", 1)[0])
        ...
        for dataset, dataset_df in prediction_df.groupby("dataset"):
            ...
            for key in keys:
                dataset_df[f"gt_{key}"] = dataset_df["annotations_average"].apply(lambda x: x.get(key, np.nan))
            ...
            for column in dataset_df.columns:
                if column.startswith("prediction_") and column != "prediction_method":
                    ...
                    pears_corr = dataset_df[column].corr(dataset_df[f"gt_{aspect_name}"], method="pearson")
                    spear_corr = dataset_df[column].corr(dataset_df[f"gt_{aspect_name}"], method="spearman")
                    kend_corr = dataset_df[column].corr(dataset_df[f"gt_{aspect_name}"], method="kendall")
                    results.append({
    ...
    results_df.sort_values(by=["dataset", "aspect", "use_reference", "score_range"], inplace=True)
